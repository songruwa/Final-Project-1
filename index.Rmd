...
ggplotly(p1)
```
According to the graph of Never smoke and Year, I found there are three outliers: Virgin Islands, Puerto Rico, and Utah. Thus, I consider them as outliers.
...
```
**
The graphs for each categories of the smoking is listed in the following order: Northeast, Southwest, West, Southwest, Midwest
**
```{r,cache=TRUE, echo=FALSE}
...
## Let's find the relationship between never smoke and state
```{r}
plot_ly(smoke,x=~Year, y= ~`Never smoked`,color = ~State, type = "scatter", mode = "lines")
```
